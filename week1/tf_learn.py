...
network = keras.Sequential([
    keras.layers.Dense(5, input_dim = 3, activation = 'tanh'),
...
# Let's compute the gradient of the weights wrt a single input
# outputTensor = network.output
# listOfVariableTensors = network.trainable_weights
# listOfVariableTensors = network.inputs
# gradients = k.gradients(outputTensor, listOfVariableTensors)
# trainingExample = np.array( [[ 0.0, 0.70, 0.0]]  )
# sess = tf.InteractiveSession()
# sess.run(tf.initialize_all_variables())
# evaluated_gradients = sess.run(gradients, feed_dict = {network.input:trainingExample})
# print("")
# print("The evaluated gradient is")
# print(evaluated_gradients)

# session = tf.Session()
# session.run(tf.global_variables_initializer())
# y_val_d_evaluated = session.run(tf.gradients(network.output, network.input), feed_dict={network.input: np.array( [[0.0, 0.81, 0.0 ]] ) })
# print(y_val_d_evaluated[0])


J = T.grad(network.output[0, 0], network.input)
jacobian = K.function( [network.input, K.learning_phase() ], [J])
